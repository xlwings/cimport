...
allow importing .c files as if they were Python modules.
...
(Distribution) for for the build can be defined by a <modulename>.cbld
file like:

# examplemod.cbld
def make_ext(modname, cfilename):
    from distutils.extension import Extension
    return Extension(name = modname,
                     sources=[cfilename, 'hello.c'],
                     include_dirs=['/myinclude'] )
def make_setup_args():
    return dict(script_args=["--compiler=mingw32"])

Extra dependencies can be defined by a <modulename>.cdep .
See README.

Since Cython 0.11, the :mod:`cimport` module also has experimental
compilation support for normal Python modules.  This allows you to
automatically run Cython on every .c and .py module that Python
...
# Performance problem: for every C file that is imported, we will 
...
# mod time of the .c is newer than the mod time of the .so but
...
def get_distutils_extension(modname, cfilename, language_level=None):
#    try:
#        import hashlib
#    except ImportError:
#        import md5 as hashlib
#    extra = "_" + hashlib.md5(open(cfilename).read()).hexdigest()  
#    modname = modname + extra
    extension_mod,setup_args = handle_special_build(modname, cfilename)
    if not extension_mod:
        if not isinstance(cfilename, str):
            # distutils is stupid in Py2 and requires exactly 'str'
            # => encode accidentally coerced unicode strings back to str
            cfilename = cfilename.encode(sys.getfilesystemencoding())
        from distutils.extension import Extension
        extension_mod = Extension(name = modname, sources=[cfilename])
        if language_level is not None:
            extension_mod.cython_directives = {'language_level': language_level}
    return extension_mod,setup_args

def handle_special_build(modname, cfilename):
    special_build = os.path.splitext(cfilename)[0] + CBLD_EXT
    ext = None
    setup_args={}
...
def handle_dependencies(cfilename):
    testing = '_test_files' in globals()
    dependfile = os.path.splitext(cfilename)[0] + CDEP_EXT
    ...
    # some of the dependencies are newer than the cfile.
    if os.path.exists(dependfile):
        ...
        # if any file that the cfile depends upon is newer than
        # the c file, 'touch' the c file so that distutils will
        # be tricked into rebuilding it.
        for file in files:
            from distutils.dep_util import newer
            if newer(file, cfilename):
                _debug("Rebuilding %s because of %s", cfilename, file)
                filetime = os.path.getmtime(file)
                os.utime(cfilename, (filetime, filetime))
                if testing:
                    _test_files.append(file)

def build_module(name, cfilename, cbuild_dir=None, inplace=False, language_level=None):
    assert os.path.exists(cfilename), (
        "Path does not exist: %s" % cfilename)
    handle_dependencies(cfilename)

    extension_mod,setup_args = get_distutils_extension(name, cfilename, language_level)
    build_in_temp=cargs.build_in_temp
    sargs=cargs.setup_args.copy()
    sargs.update(setup_args)
    build_in_temp=sargs.pop('build_in_temp',build_in_temp)

    import cbuild
    so_path = cbuild.c_to_dll(cfilename, extension_mod,
                                  build_in_temp=build_in_temp,
                                  cbuild_dir=cbuild_dir,
                                  setup_args=sargs,
                                  inplace=inplace,
                                  reload_support=cargs.reload_support)
...
def load_module(name, cfilename, cbuild_dir=None, is_package=False,
    ...
    try:
        if so_path is None:
            if is_package:
                ...
                module_name = name
            so_path = build_module(module_name, cfilename, cbuild_dir,
        ...
        if cargs.load_py_module_on_import_failure and cfilename.endswith('.py'):
            # try to fall back to normal import
            mod = imp.load_source(name, cfilename)
            assert mod.__file__ in (cfilename, cfilename+'c', cfilename+'o'), (mod.__file__, cfilename)
        else:
            import traceback
...
class CImporter(object):
    """A meta-path importer for .c files.
    """
    def __init__(self, extension=C_EXT, cbuild_dir=None, inplace=False,
                 language_level=None):
        self.extension = extension
        self.cbuild_dir = cbuild_dir
        self.inplace = inplace
        self.language_level = language_level

    def find_module(self, fullname, package_path=None):
        if fullname in sys.modules  and  not cargs.reload_support:
        ...
        try:
            ...
            if pathname and ty == imp.PKG_DIRECTORY:
                ...
                if os.path.isfile(pkg_file):
                    return CLoader(fullname, pathname,
                        init_path=pkg_file,
                        cbuild_dir=self.cbuild_dir,
            ...
            if pathname and pathname.endswith(self.extension):
                return CLoader(fullname, pathname,
                                 ...
                                 inplace=self.inplace,
                                 language_level=self.language_level)
            if ty != imp.C_EXTENSION: # only when an extension, check if we have a .c next!
                return None

            # find .c fast, when .so/.pyd exist --inplace
            cpath = os.path.splitext(pathname)[0]+self.extension
            if os.path.isfile(cpath):
                return CLoader(fullname, cpath,
                                 cbuild_dir=self.cbuild_dir,
                                 inplace=self.inplace,
                                 language_level=self.language_level)

            # .so/.pyd's on PATH should not be remote from .c's
            # think no need to implement CArgs.importer_search_remote here?
        ...
        c_module_name = module_name + self.extension
        # this may work, but it returns the file content, not its path
        #import pkgutil
        #c_source = pkgutil.get_data(package, c_module_name)
        ...
        for path in paths:
            if not path:
                ...
                path = abspath(path)
            if is_file(path+sep+c_module_name):
                return CLoader(fullname, join_path(path, c_module_name),
                                 cbuild_dir=self.cbuild_dir,
                                 inplace=self.inplace,
                                 language_level=self.language_level)

        # not found, normal package, not a .c file, none of our business
        _debug("%s not found" % fullname)
        return None

class PyImporter(CImporter):
    """A meta-path importer for normal .py files.
    """
    def __init__(self, cbuild_dir=None, inplace=False, language_level=None):
        if language_level is None:
            language_level = sys.version_info[0]
        self.super = super(PyImporter, self)
        self.super.__init__(extension='.py', cbuild_dir=cbuild_dir, inplace=inplace,
                            language_level=language_level)
        self.uncompilable_modules = {}
        self.blocked_modules = ['Cython', 'cbuild', 'cimport.cbuild',
...
class CLoader(object):
    def __init__(self, fullname, path, init_path=None, cbuild_dir=None,
                 inplace=False, language_level=None):
        _debug("CLoader created for loading %s from %s (init path: %s)",
    ...
    def load_module(self, fullname):
        ...
        if self.init_path:
            ...
            module = load_module(fullname, self.init_path,
                                 self.cbuild_dir, is_package=True,
...
def _have_importers():
    ...
    has_c_importer = False
    for importer in sys.meta_path:
        if isinstance(importer, CImporter):
            if isinstance(importer, PyImporter):
                has_py_importer = True
            else:
                has_c_importer = True

    return has_py_importer, has_c_importer

def install(cimport=True, pyimport=False, build_dir=None, build_in_temp=True,
            setup_args={}, reload_support=False,
            load_py_module_on_import_failure=False, inplace=False,
            language_level=None):
    """Main entry point. Call this to install the .c import hook in
    ...
    By default, compiled modules will end up in a ``.cbld``
    ...
    build_in_temp can be overriden by <modulename>.cbld/make_setup_args()
    ...
    <modulename>.cbld/make_setup_args()

    ``reload_support``:  Enables support for dynamic
    reload(<cmodulename>), e.g. after a change in the Cython code.
    ...
    runtime for .py files and Py2 for .c files.
    """
    if not build_dir:
        build_dir = os.path.join(os.path.expanduser('~'), '.cbld')
        
    global cargs
    cargs = CArgs()  #$pycheck_no
    cargs.build_dir = build_dir
    cargs.build_in_temp = build_in_temp
    cargs.setup_args = (setup_args or {}).copy()
    cargs.reload_support = reload_support
    cargs.load_py_module_on_import_failure = load_py_module_on_import_failure

    has_py_importer, has_c_importer = _have_importers()
    py_importer, c_importer = None, None

    if pyimport and not has_py_importer:
        py_importer = PyImporter(cbuild_dir=build_dir, inplace=inplace,
                                 language_level=language_level)
    ...
    if cimport and not has_c_importer:
        c_importer = CImporter(cbuild_dir=build_dir, inplace=inplace,
                                   language_level=language_level)
        sys.meta_path.append(c_importer)

    return py_importer, c_importer

def uninstall(py_importer, c_importer):
    """
    ...
    try:
        sys.meta_path.remove(c_importer)
    except ValueError:
        pass
